...
import { notifyButton } from "../redux/notifications/actions";

function Notification() {

  const [notifyButtonState, setNotifyButtonState] = useState(false);

  const dispatch = useDispatch()
  const countNotifys = useSelector((state) => state.notifys.notifications.length)
  ...
  return (
    <>
      <button data-count={countNotifys} className={countNotifys > 0 ? "buttonNotification buttonNotificationCount" : "buttonNotification"} onClick={() => handleClick()} style={window.location.pathname === "/dashboard" ? { top: "97px" } : { top: "55px" }}>
      ...
      {notifyButtonState ? <NotificationWrapper /> : null}
    </>
  )
}
